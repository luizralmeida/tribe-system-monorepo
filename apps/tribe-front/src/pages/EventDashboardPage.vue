  ...
  <div class="space-y-10">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6">
      <div v-for="(stat, idx) in [
        ...
        'glass-card p-4 md:p-6 border border-white/10 dark:border-white/5 group flex flex-row md:flex-col items-center md:items-start gap-3 md:gap-4',
        idx === 4 ? 'col-span-2 md:col-span-1' : ''
      ]">
        <div :class="[stat.color, 'w-10 h-10 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 shrink-0']">
          <component :is="stat.icon" class="w-5 h-5" />
        </div>
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-0.5 md:mb-1">{{ stat.label }}</p>
          <p class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white leading-tight">{{ stat.value }}</p>
        </div>
      </div>
    </div>
      ...
      <div class="glass-card overflow-hidden border border-white/10 dark:border-white/5">
        ...
        <div v-else>
                  ...
                  <td class="px-8 py-6">
                    <div class="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div v-if="guest.phone" class="flex items-center gap-4 mt-1">
                          <a :href="`https://wa.me/55${guest.phone}`" target="_blank" class="flex items-center gap-1.5 text-xs font-bold text-slate-400 hover:text-emerald-500 transition-colors">
          ...
          <div class="md:hidden divide-y divide-slate-100 dark:divide-white/5">
            <div v-for="guest in guests" :key="guest.id" class="p-6 space-y-4">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <p class="font-black text-slate-900 dark:text-white text-lg tracking-tight">{{ guest.name }}</p>
                    <p v-if="guest.companionCount > 0" class="text-[10px] font-black uppercase tracking-widest text-slate-400">
                      +{{ guest.companionCount }} acompanhantes
                    </p>
                  </div>
                </div>
                <button 
                  ...
                  :class="[
                    ...
                    guest.attended 
                      ...
                      : 'bg-transparent border-slate-200 dark:border-white/10 text-slate-300'
                  ]"
                >
              ...
              <div class="flex items-center justify-between pt-2">
                <div class="flex gap-2">
                  ...
                  <a v-if="guest.phone" :href="`https://wa.me/55${guest.phone}`" target="_blank" class="w-10 h-10 flex items-center justify-center bg-emerald-500/10 text-emerald-600 rounded-xl border border-emerald-500/20">
                    <Phone class="w-4 h-4" />
                  </a>
                </div>

                <div class="flex gap-1">
                  <button v-if="guest.status === GuestStatus.CONFIRMED" @click="openQRCodeModal(guest)" class="p-2.5 text-slate-400 bg-slate-50 dark:bg-white/5 rounded-xl">
                    <QrCode class="w-4 h-4" />
                  </button>
                  <button v-if="guest.companionCount > 0" @click="handleViewCompanions(guest)" class="p-2.5 text-slate-400 bg-slate-50 dark:bg-white/5 rounded-xl">
                    <Users class="w-4 h-4" />
                  </button>
                  <button @click="handleEditGuest(guest)" class="p-2.5 text-slate-400 bg-slate-50 dark:bg-white/5 rounded-xl">
                    <Edit class="w-4 h-4" />
                  </button>
                  <button @click="handleDeleteGuest(guest)" class="p-2.5 text-slate-400 bg-slate-50 dark:bg-white/5 rounded-xl">
                    <Trash2 class="w-4 h-4" />
                  </button>
                </div>
              </div>
